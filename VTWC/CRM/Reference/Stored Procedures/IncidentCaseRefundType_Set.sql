CREATE PROCEDURE [Reference].[IncidentCaseRefundType_Set]
(
	@userid                 INTEGER        = 0,
	@name                   NVARCHAR(256),
...
BEGIN
    ...
    IF EXISTS (SELECT 1
               FROM [Reference].[IncidentCaseRefundType]
		   	   WHERE Name = @name)
    BEGIN
        UPDATE [Reference].[IncidentCaseRefundType]
    ...
    ELSE
    BEGIN
        INSERT INTO [Reference].[IncidentCaseRefundType]
           ([Name]
           ,[Description]
           ,[CreatedDate]
           ,[CreatedBy]
	...
	SELECT @returnid = IncidentCaseRefundTypeID
	FROM   [Reference].[IncidentCaseRefundType]
	WHERE  Name = @name
