  ...
  BEGIN
   ...
   DECLARE  @ErrorState NVARCHAR(255)
	...
	BEGIN TRY   		
		...
		DROP TABLE #Leg
		SELECT jl.JourneyLegID, Leg.jl_id AS LegID, @Now AS CreatedDate, 0 AS CreatedBy, @Now AS LastModifiedDate
		...
		INNER JOIN (SELECT   ROW_NUMBER() OVER (PARTITION BY jl_id, coach, seat_number ORDER BY DateUpdated DESC) AS RANKING
							,*
		...
		USING #Leg AS SRC
		ON TRGT.JourneyLegID = SRC.JourneyLegID
		AND TRGT.LegID = SRC.LegID
		WHEN NOT MATCHED THEN
		-- Inserting new leg information 
		INSERT (JourneyLegID, LegID, CreatedDate, CreatedBy, CreatedExtractNumber, LastModifiedDate, LastModifiedBy, LastModifiedExtractNumber
				,SeatReservation, QuietZone, InformationSourceID, SourceCreatedDate, SourceModifiedDate)
		VALUES
				(JourneyLegID, LegID, CreatedDate, CreatedBy, @dataimportdetailid, LastModifiedDate, LastModifiedBy, @dataimportdetailid
