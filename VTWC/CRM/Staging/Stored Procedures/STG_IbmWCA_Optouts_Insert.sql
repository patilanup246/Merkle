...
BEGIN
   ...
   SELECT @now = GETDATE()
	...
	SELECT @preferenceid2 = preferenceid from reference.preference where name = 'General Marketing Opt-In'
	...
	DECLARE CursorIBMWcaOptouts CURSOR READ_ONLY
		FOR 
			...
			--modified to get customerid from STG_ElectronicAddress table by comparing the email addresses

			SELECT l.CustomerID, a.Email, a.EventTimeStamp 
			FROM ibm_system.dbo.SP_EmailOptOut a INNER JOIN Staging.STG_ElectronicAddress l  on (l.Address= a.email) and (l.AddressTypeID=3) and (l.primaryInd=1)
			WHERE a.IsProcessedInd = 0
			UNION
			SELECT l.CustomerID, a.Email, a.EventTimeStamp 
			FROM ibm_system.dbo.SP_OptOut a INNER JOIN Staging.STG_ElectronicAddress l  on (l.Address= a.email) and (l.AddressTypeID=3) 
			WHERE a.IsProcessedInd = 0
			order by CustomerID, EventTimeStamp desc
			...
			BEGIN
				...
				EXEC [Staging].[STG_CustomerPreference_Update] 	@userid = @userid,   
																				...
																				@sourcemodifieddate= @sourcecreateddate,	 
				...
				EXEC [Staging].[STG_CustomerPreference_Update] 	@userid = @userid,   
																				@customerid = @customerid ,
																				@preferenceid = @preferenceid2,  --General Marketing Opt-In
																				@channelid=@channelid,
																				@value= 0,
																				@sourcecreateddate =@sourcecreateddate ,
																				@sourcemodifieddate= @sourcecreateddate,	 
	...
	INNER JOIN Staging.STG_ElectronicAddress l  on (l.Address= a.email) and (l.AddressTypeID=3) and (l.primaryInd=1)
	WHERE    a.IsProcessedInd = 0
	...
	--INNER JOIN Staging.STG_KeyMapping b on a.CustomerID = b.CustomerID 
	INNER JOIN Staging.STG_ElectronicAddress l  on (l.Address= a.email) and (l.AddressTypeID=3) 
	WHERE    a.IsProcessedInd = 0
