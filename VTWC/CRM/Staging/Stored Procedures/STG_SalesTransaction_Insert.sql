...
BEGIN
	...
	SELECT originatingsystem AS ShortCode, originatingsystemtype AS [Name], @InformationSourceID AS InformationSourceID
	INTO #FulfilmentMethod
	FROM PreProcessing.TOCPLUS_Transaction
	WHERE DataImportDetailID = @DataImportDetailID
	GROUP BY originatingsystem, originatingsystemtype
   ...
   SELECT @now = GETDATE()
	...
	SET 
		...
		,FulfilmentDate = b.cmddateupdated   --DateCreated --,Staging.SetUKTime(a.DateCreated)
		...
		,SourceModifiedDate = b.cmddateupdated
	...
	LEFT  JOIN Reference.FulfilmentMethod  d ON (b.originatingsystem) = (d.ShortCode)
   ...
   AND   a.SourceModifiedDate <= b.Dateupdated
	...
	SELECT      
		...
		FROM  [PreProcessing].[TOCPLUS_Transaction] a --PreProcessing.MSD_SalesOrder a
				...
				LEFT  JOIN Reference.FulfilmentMethod    d ON (a.originatingsystem) = (d.ShortCode)
					     								             AND  d.InformationSourceID = @informationsourceid
	...
	/*Commenting to address JIRA - CLT5001_65054-64 
	...
	End Commenting to address JIRA - CLT5001_65054-64 */
