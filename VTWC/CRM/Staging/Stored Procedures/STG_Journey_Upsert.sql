...
=================================================================================================*/
  ...
  BEGIN
   ...
   DECLARE  @ErrorState NVARCHAR(255)
	...
	IF OBJECT_ID(N'tempdb..#FareSettingTOC') IS NOT NULL
	DROP TABLE #FareSettingTOC
	...
	EXEC [Reference].[FareSettingTOC_Upsert] 
	...
	/*
	NEED TO APPLY RANKING TO PICK ONE RECORD BECAUSE SAME CODE WAS COMING THROUGH WITH SIMILAR DESCRIPTION AS BELOW
	1DF	ADVANCE SINGLE (1ST CLASS)
	1DF	FIRST ADVANCE SINGLE
	*/
	...
	INTO #Product
	FROM (
	SELECT ROW_NUMBER() OVER (PARTITION BY  tickettypecode ORDER BY tickettypecode) AS Ranking
		   ,tickettypecode AS TicketTypeCode, tickettypedesc AS [Name], COALESCE(TC.TicketClassID,-1) AS TicketClassID
		   ,1 AS InformationSourceID	
	FROM PreProcessing.TOCPLUS_Journey AS J
	LEFT JOIN Reference.TicketClass AS TC
		ON J.class = TC.ShortCode
	WHERE DataImportDetailID = @DataImportDetailID
	GROUP BY tickettypecode, tickettypedesc, COALESCE(TC.TicketClassID,-1)) AS SQ 
	WHERE Ranking = 1
	...
	BEGIN TRY   		
		...
		DROP TABLE #Journey
		SELECT  J.journeyid AS ExtReference, J.purchaseid , SD.SalesDetailID, SD.SalesTransactionID, SD.CustomerID, J.tcstransactionid, J.tcsbookingid, TOC.TOCID, Origin.LocationID AS OrigStationID
			   ,Dest.LocationID AS DestStationID, J.outdatedep, J.outdatearr, CASE WHEN YEAR(J.retdatedep)='1899' THEN NULL ELSE J.retdatedep END AS retdatedep
			   ,CASE WHEN YEAR(J.retdatearr)='1899' THEN NULL ELSE J.retdatearr END AS retdatearr, CASE WHEN J.journeytype = 'R' THEN 1 ELSE 0 END AS IsReturnIndicator
		...
		LEFT JOIN Reference.Product AS P
			...
			AND P.TicketClassID = TC.TicketClassID
		...
		LEFT JOIN #tmp_NLCCode_LU AS Origin WITH (NOLOCK)
			ON Origin.CRSCode = J.origstationcode
		LEFT JOIN #tmp_NLCCode_LU AS Dest WITH (NOLOCK)
			ON Dest.CRSCode = J.deststationcode
		...
		INSERT (SalesDetailID, CustomerID, SalesTransactionID, ExtReference, TCSBookingID, TOCIDPrimary, LocationIDOrigin, LocationIDDestination
			,[OutDepartureDateTime], [OutArrivalDateTime], [RetDepartureDateTime], [RetArrivalDateTime], IsReturnInd,InformationSourceID, SourceCreatedDate
			,SourceModifiedDate, RailCard1, NoOfRailCards, TotalAdults, TotalChildren, TotalReturningAdults , totalreturningchildren, costoftickets
		...
		VALUES
				...
				,outdatedep, outdatearr, retdatedep, retdatearr, IsReturnIndicator, @InformationSourceID, DateCreated
				,SourceModifiedDate, RailCard, NoOfRailCards, TotalAdults, TotalChildren, TotalReturningAdults , totalreturningchildren, costoftickets
				,totalcost, savingsmade, procode, JourneyReference,IsOutboundInd, outboundmileage, AvailabilityCodeID
				,DisabledInd, NoFullFareAdults, NoDiscFareAdults, NoFullFareChildren, NoDiscFareChildren, NoRailcard1, DateCreated
				,DateUpdated, FareId, PromoCode, FullAdultFare, DiscAdultFare1, DiscAdultFare2, DiscAdultFare3, FullChildFare
				,DiscChildFare1, DiscChildFare2, DiscChildFare3, NoAdultsFullFare, NoAdultsDiscFare1, NoAdultsDiscFare2, NoAdultsDiscFare3
		...
		UPDATE SD
		SET SD.ProductID = COALESCE(J.ProductID,-1)
