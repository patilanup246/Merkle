...
=================================================================================================*/
  ...
  BEGIN
   ...
   DECLARE  @ErrorState NVARCHAR(255)
	...
	DROP TABLE #Product
	SELECT SupplementTypeCode AS TicketTypeCode, SupplementTypeDesc AS [Name], -1 AS TicketClassID, @InformationSourceID AS InformationSourceID
	INTO #Product
	...
	BEGIN TRY   		
		...
		FROM (
			...
			LEFT JOIN Reference.Product AS P
				ON SUPP.SupplementTypeCode = P.TicketTypeCode
				AND P.TicketClassID = -1) AS SQ
		WHERE Ranking =1 
		...
		INSERT (SalesTransactionID, ProductID, Quantity, UnitPrice, SalesAmount, IsTrainTicketInd
				,ExtReference, TCSBookingID, InformationSourceID, CustomerID, CreatedDate, CreatedBy, CreatedExtractNumber
				,LastModifiedDate, LastModifiedBy, LastModifiedExtractNumber)
		VALUES
				(SalesTransactionID, ProductID, Quantity, UnitPrice, SalesAmount, 0
				,ExtReference, TCSBookingID, InformationSourceID, CustomerID, DateCreated, 0, @DataImportDetailID
