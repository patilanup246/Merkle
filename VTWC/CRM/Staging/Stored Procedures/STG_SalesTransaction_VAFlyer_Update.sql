/*===========================================================================================
Name:			STG_SalesTransaction_VAFlyer_Update
Purpose:		
...
Peer Review:	
Call script:	e. g, EXEC Staging.STG_SalesTransaction_VAFlyer_Update 0, 429
=================================================================================================*/

CREATE PROCEDURE [Staging].[STG_SalesTransaction_VAFlyer_Update]
(
	@userid                INTEGER = 0,
...
BEGIN
	...
	SELECT @StepName = 'Staging.STG_SalesTransaction_VAFlyer_Update ProcedureStart'
   ...
   SELECT @importfilename = importfilename FROM [Operations].[DataImportDetail] WHERE dataimportdetailid = @dataimportdetailid
	...
	SELECT @LoyaltyProgrammeTypeID= LoyaltyProgrammeTypeID FROM reference.[LoyaltyProgrammeType] WHERE name = 'Virgin Flying Club'
										
    UPDATE a
    SET LoyaltyReference = b.mem_number,
		LoyaltyProgrammeTypeID = @LoyaltyProgrammeTypeID,
		LoyaltySchemeName ='Virgin Flying Club',
		LastModifiedExtractNumber = @dataimportdetailid
	FROM  [Staging].[STG_SalesTransaction] a
	INNER JOIN   preprocessing.TOCPLUS_VAFlyerTrans b on a.ExtReference = CAST(b.tr_id AS NVARCHAR(256)) 
	...
	INNER JOIN   [Staging].[STG_SalesTransaction] b on b.ExtReference = CAST(a.tr_id AS NVARCHAR(256)) 
   ...
   FROM   PreProcessing.TOCPLUS_VAFlyerTrans
	...
	SELECT @StepName = 'Staging.STG_SalesTransaction_VAFlyer_Update Procedure Try'
