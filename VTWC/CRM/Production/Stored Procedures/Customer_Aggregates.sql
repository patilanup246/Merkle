 ...
 */
    ...
    BEGIN
        ...
        EXEC [synUspAudit] @AuditType = 'TRACE START', @Process = @spname, @ProcessStep = 'Pre execute tasks', @DatabaseName = 'CRM', @FileName = '', @Rows = NULL, @PrintToScreen = 0;
        ...
        SELECT TOP 1
                @ticket_first = [tc].[TicketClassID]
        FROM    [Reference].[TicketClass] AS [tc]
        WHERE   [tc].[Name] = 'First'
        ORDER BY [tc].[LastModifiedBy] DESC;

        SELECT TOP 1
                @ticket_standard = [tc].[TicketClassID]
        FROM    [Reference].[TicketClass] AS [tc]
        WHERE   [tc].[Name] = 'Standard'
        ORDER BY [tc].[LastModifiedBy] DESC;
	--Log start time--

        EXEC [synUspAudit] @AuditType = 'TRACE END', @Process = @spname, @ProcessStep = 'Pre execute tasks', @DatabaseName = 'CRM', @FileName = '', @Rows = NULL, @PrintToScreen = 0; 
		
        EXEC [Operations].[LogTiming_Record] @userid = @userid,
            @logsource = @spname, @logtimingidnew = @logtimingidnew OUTPUT;


		EXEC [synUspAudit] @AuditType = 'TRACE START', @Process = @spname, @ProcessStep = 'TableBackup    Staging.STG_CustomerAggregation', @DatabaseName = 'CRM', @FileName = '', @Rows = NULL, @PrintToScreen = 0;

        EXEC [Operations].[TableBackup] @table_name = N'Staging.STG_CustomerAggregation';

		EXEC [synUspAudit] @AuditType = 'TRACE END', @Process = @spname, @ProcessStep = 'TableBackup    Staging.STG_CustomerAggregation', @DatabaseName = 'CRM', @FileName = '', @Rows = NULL, @PrintToScreen = 0; 

		EXEC [synUspAudit] @AuditType = 'TRACE START', @Process = @spname, @ProcessStep = 'Insert into STG_CustomerAggregation & CheckTableAndRestoreOnFailure', @DatabaseName = 'CRM', @FileName = '', @Rows = NULL, @PrintToScreen = 0;
	...
	-- Delete old aggregates
        ...
        FROM    (
                 SELECT [b].[CustomerID]
                       ,COUNT(DISTINCT CASE WHEN [b].[SalesTransactionDate] > @12MonthsAgo
                       ...
                       ,COUNT(DISTINCT CASE WHEN [b].[SalesTransactionDate] > @6MonthsAgo
                                                 AND [b].[SalesTransactionDate] < @today
                                            THEN [b].[SalesTransactionID]
                                       END) AS [Trans_L6M]
                       ,COUNT(DISTINCT CASE WHEN [b].[SalesTransactionDate] > @6MonthsAgo
                       ...
                       ,COUNT(DISTINCT CASE WHEN [b].[SalesTransactionDate] > @6MonthsAgo
                                                 ...
                                                 AND (DATEPART(WEEKDAY,
                                                              [c].[OutTravelDate]) BETWEEN 2 AND 6
                                                      OR DATEPART(WEEKDAY,
                                                              [c].[ReturnTravelDate]) BETWEEN 2 AND 6
                                                     )
                                            THEN [b].[SalesTransactionID]
                 ...
                 INNER JOIN [Staging].[STG_SalesDetail] [c]
                 ON     [b].[SalesTransactionID] = [c].[SalesTransactionID]
                 INNER JOIN [Reference].[Product] [d]
        ...
        EXEC [Operations].[CheckTableAndRestoreOnFailure] @table_name = N'Staging.STG_CustomerAggregation';

		EXEC [synUspAudit] @AuditType = 'TRACE END', @Process = @spname, @ProcessStep = 'Insert into STG_CustomerAggregation & CheckTableAndRestoreOnFailure', @DatabaseName = 'CRM', @FileName = '', @Rows = @recordcount, @PrintToScreen = 0; 
	...
	--VTWC SPECIFIC
		...
		EXEC [synUspAudit] @AuditType = 'TRACE START', @Process = @spname, @ProcessStep = 'Populate temp table - VtwcSales', @DatabaseName = 'CRM', @FileName = '', @Rows = NULL, @PrintToScreen = 0;
	...
	--Selects all sales detail IDs which have at least 1 VTWC ticket associated with them
        ...
        INNER JOIN [Staging].[STG_Journey] [jo]
        ON      [jo].[SalesDetailID] = [c].[SalesDetailID]
        INNER JOIN [Staging].[STG_JourneyLeg] [jl]
        ON      [jo].[JourneyID] = [jl].[JourneyID]
                AND [TOCID] = 31
		...
		EXEC [synUspAudit] @AuditType = 'TRACE END', @Process = @spname, @ProcessStep = 'Populate temp table - VtwcSales', @DatabaseName = 'CRM', @FileName = '', @Rows = @recordcount, @PrintToScreen = 0; 

		EXEC [synUspAudit] @AuditType = 'TRACE START', @Process = @spname, @ProcessStep = 'TableBackup    Staging.STG_VTWC_CustomerAggregation', @DatabaseName = 'CRM', @FileName = '', @Rows = NULL, @PrintToScreen = 0;

        EXEC [Operations].[TableBackup] @table_name = N'Staging.STG_VTWC_CustomerAggregation';

		EXEC [synUspAudit] @AuditType = 'TRACE END', @Process = @spname, @ProcessStep = 'TableBackup    Staging.STG_VTWC_CustomerAggregation', @DatabaseName = 'CRM', @FileName = '', @Rows = NULL, @PrintToScreen = 0; 

		EXEC [synUspAudit] @AuditType = 'TRACE START', @Process = @spname, @ProcessStep = 'Insert into STG_VTWC_CustomerAggregation & CheckTableAndRestoreOnFailure', @DatabaseName = 'CRM', @FileName = '', @Rows = NULL, @PrintToScreen = 0;
	...
	--VTWC Specific
			...
			--Journey dates
               ...
               ,CASE WHEN [vtwc].[VTWC_DateLastTravelAny_OUT] > [vtwc].[VTWC_DateLastTravelAny_RET]
                     THEN [vtwc].[VTWC_DateLastTravelAny_OUT]
                     ELSE [vtwc].[VTWC_DateLastTravelAny_RET]
                END AS [VTWC_DateLastTravelAny]
               ,CASE WHEN [vtwc].[VTWC_DateNextTravelAny_OUT] > [vtwc].[VTWC_DateNextTravelAny_RET]
               ...
               ,CASE WHEN [vtwc].[VTWC_DateLastTravelFirst_OUT] > [vtwc].[VTWC_DateLastTravelFirst_RET]
                     THEN [VTWC_DateLastTravelFirst_OUT]
                     ELSE [vtwc].[VTWC_DateLastTravelFirst_RET]
                END AS [VTWC_DateLastTravelFirst]
               ,CASE WHEN [vtwc].[VTWC_DateNextTravelFirst_OUT] > [vtwc].[VTWC_DateNextTravelFirst_RET]
               ...
               ,CASE WHEN [vtwc].[VTWC_DateLastTravelStandard_OUT] > [vtwc].[VTWC_DateLastTravelStandard_RET]
        ...
        FROM    (
                 SELECT [b].[CustomerID]
                       ...
                       ,
						-- VTWC_DateFirstPurchaseFirst
                        MIN(CASE WHEN [d].[TicketClassID] = @ticket_first
                                 THEN [b].[SalesTransactionDate]
                            END) AS [VTWC_DateFirstPurchaseFirst]
                       ,
						-- VTWC_DateLastPurchaseFirst
                        MAX(CASE WHEN [d].[TicketClassID] = @ticket_first
                                 THEN [b].[SalesTransactionDate]
                            END) AS [VTWC_DateLastPurchaseFirst]
                       ,
						-- VTWC_DateFirstPurchaseStandard
                        MIN(CASE WHEN [d].[TicketClassID] = @ticket_standard
                                 THEN [b].[SalesTransactionDate]
                            END) AS [VTWC_DateFirstPurchaseStandard]
                       ,
						-- VTWC_DateLastPurchaseStandard
                        MAX(CASE WHEN [d].[TicketClassID] = @ticket_standard
                                 THEN [b].[SalesTransactionDate]
                            END) AS [VTWC_DateLastPurchaseStandard]
                       ,
						-- VTWC_DateFirstTravelAny
                        MIN(CASE WHEN [c].[OutTravelDate] < @today
                                 THEN [c].[OutTravelDate]
                            END) AS [VTWC_DateFirstTravelAny]
                       ,

						--VTWC_DateLastTravelAny
                        MAX(CASE WHEN [c].[OutTravelDate] < @today
                                 THEN [c].[OutTravelDate]
                            END) AS [VTWC_DateLastTravelAny_OUT]
                       ,MAX(CASE WHEN [c].[ReturnTravelDate] IS NOT NULL
                       ...
                       ,MIN(CASE WHEN [c].[ReturnTravelDate] IS NOT NULL
                                      AND [c].[ReturnTravelDate] > @today
                                 THEN [c].[ReturnTravelDate]
                       ...
                       ,SUM(CASE WHEN [b].[SalesTransactionDate] > DATEADD(MM, -3,
                                                              @today)
                                 THEN [c].[SalesAmount]
                            END) AS [VTWC_SalesAmount3Mnth]
                       ,SUM(CASE WHEN [b].[SalesTransactionDate] > @6MonthsAgo
                                 THEN [c].[SalesAmount]
                            END) AS [VTWC_SalesAmount6Mnth]
                       ,SUM(CASE WHEN [b].[SalesTransactionDate] > @12MonthsAgo
                                 THEN [c].[SalesAmount]
                            END) AS [VTWC_SalesAmount12Mnth]
                       ,COUNT(DISTINCT [b].[SalesTransactionID]) AS [VTWC_SalesTransactionTotal]
                       ,COUNT(DISTINCT CASE WHEN [b].[SalesTransactionDate] > DATEADD(MM,
                                                              -1, @today)
                                            THEN [b].[SalesTransactionID]
                 ...
                 INNER JOIN [#VtwcSales] [v]
                 ON     [v].[SalesDetailID] = [c].[SalesDetailID]
        ...
        SELECT  @recordcount+= @@ROWCOUNT;
        EXEC [Operations].[CheckTableAndRestoreOnFailure] @table_name = N'Staging.STG_VTWC_CustomerAggregation';
	
		EXEC [synUspAudit] @AuditType = 'TRACE END', @Process = @spname, @ProcessStep = 'Insert into STG_VTWC_CustomerAggregation & CheckTableAndRestoreOnFailure', @DatabaseName = 'CRM', @FileName = '', @Rows = @recordcount, @PrintToScreen = 0;
