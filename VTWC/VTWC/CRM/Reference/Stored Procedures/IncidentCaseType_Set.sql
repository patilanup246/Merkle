CREATE PROCEDURE [Reference].[IncidentCaseType_Set]
(
	@userid                 INTEGER        = 0,
	@name                   NVARCHAR(256),
...
BEGIN
    ...
    IF EXISTS (SELECT 1
               FROM [Reference].[IncidentCaseType]
		   	   WHERE Name = @name)
    BEGIN
        UPDATE [Reference].[IncidentCaseType]
    ...
    ELSE
    BEGIN
        INSERT INTO [Reference].[IncidentCaseType]
           ([Name]
           ,[Description]
           ,[CreatedDate]
           ,[CreatedBy]
	...
	SELECT @returnid = IncidentCaseTypeID
	FROM   [Reference].[IncidentCaseType]
	WHERE  Name = @name
