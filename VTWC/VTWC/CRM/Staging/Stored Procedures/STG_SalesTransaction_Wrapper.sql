...
/*===========================================================================================
Name:			STG_SalesTransaction_Wrapper
Purpose:		
...
Peer Review:	
Call script:	EXEC STG_SalesTransaction_Wrapper 0
=================================================================================================*/

CREATE PROCEDURE [Staging].[STG_SalesTransaction_Wrapper]
(
    @userid                INTEGER = 0
)
AS
BEGIN
	...
	SELECT @StepName = 'Staging.STG_SalesTransaction_Wrapper'
	...
	IF CURSOR_STATUS('global','GetLogId_SalesTransactions')>=-1
   BEGIN
	CLOSE GetLogId_SalesTransactions
	DEALLOCATE GetLogId_SalesTransactions  
	END

	IF CURSOR_STATUS('global','SalesTransactions')>=-1
   BEGIN
	CLOSE SalesTransactions
	DEALLOCATE SalesTransactions  
	END  

	DECLARE GetLogId_SalesTransactions CURSOR READ_ONLY
   FOR 
	...
	INNER JOIN PreProcessing.TOCPLUS_Transaction d ON d.DataImportDetailID = e.DataImportDetailID 
	WHERE (b.Name = 'Imported') 
	AND   c.Name = @dataimporttype

	

	OPEN GetLogId_SalesTransactions

	FETCH NEXT FROM GetLogId_SalesTransactions 		INTO @dataimportlogid
   ...
   BEGIN
    ...
    FOR 
		...
		AND b.NAME = 'TOC Plus Transaction'
        ...
        BEGIN
		    ...
		    EXEC [Staging].[STG_SalesTransaction_Insert] @userid             = @userid,
...
/*
   ...
   DEALLOCATE GetLogId_SalesTransactions
	...
	BEGIN CATCH  
	 ...
	 SELECT @StepName = 'STG_SalesTransaction_Insert'
