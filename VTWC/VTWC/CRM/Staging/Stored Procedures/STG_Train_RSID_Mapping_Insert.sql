...
BEGIN
	...
	--select
	--	convert(date, [Staging].[GetUKTime](j.DepartureDateTime)) [TravelDate],
	--	left(convert(time(0), [Staging].[GetUKTime](j.DepartureDateTime)),5) [TravelTime],
	--	l.rsid [RSID],
	--	l.LocationIDOrigin [OriginID],
	--	l.LocationIDDestination [DestID]
	--	into #journeys
	--	from [Staging].[STG_Journey] j
	--	inner join Staging.stg_journeyLeg l on j.journeyid=l.JourneyID and l.tocid=9 and j.LocationIDOrigin=l.LocationIDOrigin
	--	where j.DepartureDateTime between convert(date,dateadd(mm,-6,getdate())) and convert(date,dateadd(day,+14,getdate()));
	...
	--inner join [Rail_Timetable].[dbo].JourneyStages s with(nolock)
	--  on s.toc='GR'				-- VTEC
	...
	--inner join [Rail_Timetable].[dbo].[JourneyStages] d with(nolock)
