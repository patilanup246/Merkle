CREATE PROCEDURE [PreProcessing].[CBE_RefundReasonCode_Insert]
(
	@userid                INTEGER = 0,
...
BEGIN
	...
	;WITH CTE_RefundReasonCode AS (
	              SELECT 	[CBE_RefundReasonCodeID],
							[ID],
							[Code],
							[Description],
							[Is_Rejection],
							...
							ROW_NUMBER() OVER (partition by [Code] ORDER BY [Date_Modified] DESC
						                                                   ,[CBE_RefundReasonCodeID] DESC) RANKING
                  FROM   [PreProcessing].[CBE_RefundReasonCode]
				  WHERE  DataImportDetailID = @dataimportdetailid
	              AND    ProcessedInd = 0)

    SELECT *
	INTO #tmp_RefundReasonCode
	FROM CTE_RefundReasonCode
    WHERE Ranking = 1

	--Refund Reason Codes in [Reference].[RefundReasonCode] but not in #tmp_RefundReasonCode are to be treated as expired references
    ...
    FROM [Reference].[RefundReasonCode] a
	LEFT JOIN #tmp_RefundReasonCode     b ON a.Code = b.Code
	...
	FROM #tmp_RefundReasonCode b
	INNER JOIN [Reference].[RefundReasonCode] a ON a.Code                 = b.Code
    ...
    FROM [Reference].[RefundReasonCode] a
	INNER JOIN PreProcessing.CBE_RefundReasonCode b ON a.ExtReference = b.code 
	                                                AND a.InformationSourceID = @informationsourceid
    ...
    INSERT INTO [Reference].[RefundReasonCode]
        (Name,
		 Description,
		 CreatedDate,
		 CreatedBy,
    ...
    SELECT  a.Description,
			...
			a.Code,
			a.Date_Created,
			CASE WHEN a.Is_Active = 1 THEN @maxvalidityend ELSE a.Date_Modified END
    FROM #tmp_RefundReasonCode a
	LEFT JOIN Reference.RefundReasonCode b ON b.Code                 = a.Code
